    ...
    <a href="edit.php?post_type=wpdmpro&page=orders&task=vieworder&id=<?php echo $order->order_id; ?>" class="list-group-item">
        <div class="media">
            ...
            <div class="media-body">
                ...
                <small><?php echo wp_date(get_option('date_format')." ".get_option('time_format'), $order->date) ?> | <?php $u = get_user_by('id',$order->uid); echo is_object($u) && !is_wp_error($u)?$u->user_email:$order->billing_info['order_email']; ?></small>
            </div>
        </div>
    </a>
    <?php } ?>
